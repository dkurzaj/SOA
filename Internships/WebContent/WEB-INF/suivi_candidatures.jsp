	  		...
	  		<tbody>
	  		    <c:forEach items="${liste}" var="postuler">
		  		    <tr>
						...
						<td class="vert-align">
		        			<h2 class="list-group-item-heading"><a href="details">${postuler.getEtud().getPrenom()} ${postuler.getEtud().getNom()}</a></h3>
		        			...
		        			<form method="get" action="aa">
								...
								<a href="#?action=accepter&idoffre=${postuler.getIdPostuler()}" class="btn btn-success">Accepter la demande</a>
								<a href="#?action=refuser&idoffre=${postuler.getIdPostuler()}"type="button" class="btn btn-success">Refuser la demande</button>										
							</form>
						</td>
		        	</tr>
				</c:forEach>
